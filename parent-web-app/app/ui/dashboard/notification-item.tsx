...
import { fetchNotificationsList, NotificationType } from '@/app/src/api/services/notificationService';
    ...
    const loadNotifications = async () => {
      try {
        ...
        if (notificationsArray.length > 0) {
          // Use real backend data - take only the first 4 notifications
          setNotifications(notificationsArray.slice(0, 4));
          setUseMockData(false);
        } else {
          // Only use mock data if no real notifications are available
  ...
  return (
    <div className="space-y-2">
      {notifications.map((notification, index) => {
        // Ensure each notification has a unique key
        const uniqueKey = notification.id || notification._id || `notification-${index}`;
        
        return (
          <div
            key={uniqueKey}
            className="flex items-center space-x-4 p-2 mb-2 w-full cursor-pointer hover:bg-gray-50 rounded-lg transition-colors"
